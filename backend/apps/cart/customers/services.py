...
class CustomerCartService:
    ...
    def validate_checkout_eligibility(cls, user):
        ...
        for item in items:
            ...
            if (
                not v
                or getattr(v, "blocked", False)
                or not getattr(v, "is_active", True)
            ):
    ...
    def add_item_to_cart(cls, user, variant_id, quantity):
        ...
        if new_quantity > variant.stock_quantity:
            raise ValidationError(
                f"Only {variant.stock_quantity} items are available in stock."
            )
        ...
        if item:
            ...
            item = CartItem.objects.create(
                cart=cart, variant=variant, quantity=new_quantity
            )
    ...
    def update_cart_item(cls, user, item_id, quantity=None, action=None):
        ...
        if action == "increment":
            ...
            raise ValidationError(
                "Provide either quantity or action (increment/decrement)."
            )
