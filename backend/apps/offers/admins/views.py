...
class AdminProductOfferListCreateAPIView(APIView):
    ...
    def get(self, request):
        ...
        queryset = AdminOfferSelector.get_product_offers(
...
class AdminProductOfferDetailAPIView(APIView):
    ...
    def get(self, request, offer_id):
        ...
        if not offer:
            return Response(
                {"success": False, "message": "Product offer not found."},
                status=status.HTTP_404_NOT_FOUND,
            )
        return Response(
            {"success": True, "data": AdminProductOfferSerializer(offer).data}
        )

    def put(self, request, offer_id):
        serializer = AdminCreateUpdateProductOfferSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        offer = OfferService.update_product_offer(
            offer_id=offer_id, **serializer.validated_data
        )
        return Response(
            {
                "success": True,
    ...
    def patch(self, request, offer_id):
        ...
        if is_active is True:
            ...
            return Response(
                {"success": False, "message": "Invalid toggle request."},
    ...
    def delete(self, request, offer_id):
        ...
        return Response(
            {"success": True, "message": "Product offer deleted successfully!"},
...
class AdminCategoryOfferListCreateAPIView(APIView):
    ...
    def get(self, request):
        search = request.query_params.get("search", None)
        status_filter = request.query_params.get("status", None)
        queryset = AdminOfferSelector.get_category_offers(
            search=search, status_filter=status_filter
        )
        paginator = OfferPagination()
        page_qs = paginator.paginate_queryset(queryset, request)
...
class AdminCategoryOfferDetailAPIView(APIView):
    ...
    def get(self, request, offer_id):
        ...
        if not offer:
            return Response(
                {"success": False, "message": "Category offer not found."},
                status=status.HTTP_404_NOT_FOUND,
            )
        return Response(
            {"success": True, "data": AdminCategoryOfferSerializer(offer).data}
        )

    def put(self, request, offer_id):
        serializer = AdminCreateUpdateCategoryOfferSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        offer = OfferService.update_category_offer(
            offer_id=offer_id, **serializer.validated_data
        )
        return Response(
            {
    ...
    def delete(self, request, offer_id):
        ...
        return Response(
            {"success": True, "message": "Category offer deleted successfully!"},
            status=status.HTTP_200_OK,
        )
