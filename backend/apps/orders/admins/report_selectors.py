...
class SalesReportSelector:
    ...
    def parse_and_validate_dates(cls, start_date=None, end_date=None, date_range=None):
        ...
        if date_range:
            ...
            if range_key == "today":
                ...
                raise ValidationError(
                    {"date_range": f"Invalid date range preset '{date_range}'."}
                )
        else:
            if start_date or end_date:
                parsed_start = (
                    cls._parse_date_str(start_date, "start_date")
                    if start_date
                    else today.replace(day=1)
                )
                parsed_end = (
                    cls._parse_date_str(end_date, "end_date") if end_date else today
                )
            else:
        ...
        if (parsed_end - parsed_start).days > 366:
            raise ValidationError(
                {
                    "date_range": "Requested date range exceeds maximum allowed limit of 366 days."
                }
            )
    ...
    def _parse_date_str(cls, date_str, field_name):
        ...
        if (
            hasattr(date_str, "year")
            and hasattr(date_str, "month")
            and hasattr(date_str, "day")
        ):
            return date_str
        try:
            ...
            raise ValidationError(
                {field_name: f"Invalid date format '{date_str}'. Expected YYYY-MM-DD."}
            )
    ...
    def get_sales_summary(cls, start_dt=None, end_dt=None):
        ...
        item_agg = items_qs.aggregate(
            ...
            offer_discount=Coalesce(
                Sum(
                    (F("original_price") - F("price")) * F("quantity"),
        ...
        ).aggregate(returned_amount=Coalesce(Sum("refund_amount"), Decimal("0.00")))

        cancelled_agg = OrderCancellationRequest.objects.filter(
            order__in=orders_qs,
            status=OrderCancellationRequest.CancellationStatus.APPROVED,
        ).aggregate(cancelled_amount=Coalesce(Sum("refund_amount"), Decimal("0.00")))
    ...
    def get_sales_breakdown(cls, start_dt, end_dt, group_by="day"):
        ...
        for item in grouped:
            p_val = item["period"]
            date_str = (
                p_val.strftime("%Y-%m-%d") if hasattr(p_val, "strftime") else str(p_val)
            )

            period_orders = (
                orders_qs.filter(created_at__date=p_val)
                if hasattr(p_val, "year")
                else orders_qs
            )
            ...
            item_agg = items_qs.aggregate(
                gross_sales=Coalesce(
                    Sum(
                        F("original_price") * F("quantity"), output_field=DecimalField()
                    ),
                    Decimal("0.00"),
                ),
                offer_discount=Coalesce(
            ...
            ret_amt = (
                OrderReturnRequest.objects.filter(
                ...
                .aggregate(t=Coalesce(Sum("refund_amount"), Decimal("0.00")))["t"]
                .quantize(Decimal("0.01"))
            )
            ...
            n_sales = max(
                Decimal("0.00"), subtotal - c_disc + ship - (ret_amt + canc_amt)
            ).quantize(Decimal("0.01"))
    ...
    def get_sales_chart_data(
        cls, start_dt, end_dt, group_by="day", parsed_start=None, parsed_end=None
    ):
        ...
        if group_by == "month":
            ...
            while curr <= end_month:
                ...
                row = breakdown_map.get(month_key) or breakdown_map.get(
                    curr.strftime("%Y-%m")
                )
            ...
            results.append(
                {
                    ...
                    "revenue": (row["revenue"] or Decimal("0.00")).quantize(
                        Decimal("0.01")
                    ),
                }
            )
