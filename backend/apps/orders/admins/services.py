...
class AdminReturnRequestService:
    ...
    def approve_return(cls, return_id, admin_remark=""):
        ...
        if return_req.status != OrderReturnRequest.ReturnStatus.PENDING:
            raise ValidationError(
                {
                    "status": f"Return request is already '{return_req.get_status_display()}' and cannot be processed again."
                }
            )
        ...
        if item:
            ...
            refund_amount = (
                return_req.refund_amount
                or CustomerOrderService.calculate_item_refund(item)
            )
            if refund_amount <= 0:
        ...
        WalletService.refund(
            ...
            description=f"Refund for Return Request #{str(return_req.id)[:8].upper()}{item_desc}"
            + (f": {admin_remark}" if admin_remark else ""),
        )
        ...
        all_returned_or_cancelled = all(
            i.status in [OrderItem.ItemStatus.RETURNED, OrderItem.ItemStatus.CANCELLED]
            for i in all_items
        )
    ...
    def reject_return(cls, return_id, admin_remark=""):
        ...
        if return_req.status != OrderReturnRequest.ReturnStatus.PENDING:
            raise ValidationError(
                {
                    "status": f"Return request is already '{return_req.get_status_display()}'."
                }
            )
        ...
        if return_req.order_item:
            ...
            return_req.order
            and return_req.order.order_status == Order.OrderStatus.RETURN_REQUESTED
        ):
...
class AdminCancellationRequestService:
    ...
    def approve_cancellation(cls, cancellation_id, admin_remark="", admin_user=None):
        try:
            ...
            raise ValidationError(
                {"cancellation_id": "Cancellation request not found."}
            )

        if canc_req.status != OrderCancellationRequest.CancellationStatus.PENDING:
            raise ValidationError(
                {
                    "status": f"Cancellation request is already '{canc_req.get_status_display()}' and cannot be processed again."
                }
            )
        ...
        if order.order_status in non_cancellable:
            raise ValidationError(
                f"Cannot approve cancellation because order status is '{order.get_order_status_display()}'."
            )
    ...
    def reject_cancellation(cls, cancellation_id, admin_remark="", admin_user=None):
        ...
        if canc_req.status != OrderCancellationRequest.CancellationStatus.PENDING:
            raise ValidationError(
                {
                    "status": f"Cancellation request is already '{canc_req.get_status_display()}'."
                }
            )
