    ...
    def place_order(cls, user, address_id, payment_method="COD"):
        ...
        if not contact_phone or not contact_phone.isdigit() or len(contact_phone) != 10:
            raise ValidationError(
                {
                    "phone": "A valid 10-digit contact phone number is required before placing an order."
                }
            )
        ...
        if payment_method not in allowed_methods:
            raise ValidationError(
                {"payment_method": f"Unsupported payment method '{payment_method}'."}
            )
        ...
        for item in cart_items:
            ...
            if not variant:
                raise ValidationError(
                    {"cart": "An item in your cart no longer exists."}
                )

            if not variant.is_active or variant.blocked:
                raise ValidationError(
                    {"cart": f"Variant '{variant.variant_name}' is unavailable."}
                )
            ...
            if not product or not product.is_active or product.blocked:
                raise ValidationError(
                    {
                        "cart": f"Product '{variant.product.name if product else 'Item'}' is unavailable."
                    }
                )
        ...
        total_amount = (
            checkout_calc["final_payable"]
            if payment_method != Order.PaymentMethod.WALLET
            else checkout_calc["total_with_shipping"]
        )
        ...
        order = Order.objects.create(
            ...
            coupon_code=(
                applied_coupon.code if (applied_coupon and coupon_discount > 0) else ""
            ),
...
class CustomerOrderService:
    ...
    def request_item_cancellation(cls, user, item_id, reason=None, description=None):
        ...
        if order.order_status in non_cancellable:
            raise ValidationError(
                f"Item cannot be cancelled because order status is '{order.get_order_status_display()}'."
            )
    ...
    def cancel_order(cls, user, order_id, reason=None):
        ...
        active_items = list(
            order.items.select_related("variant").filter(
                status=OrderItem.ItemStatus.ACTIVE
            )
        )
    ...
    def cancel_order_item(cls, user, item_id, reason=None):
        ...
        try:
            item = OrderItem.objects.select_related("order", "variant").get(
                id=item_id, order__user=user
            )
        except OrderItem.DoesNotExist:
            raise ValidationError("Order item not found or unauthorized.")
        ...
        if active_items.exists():
            ...
            order.shipping_fee = (
                Decimal("0.00")
                if (order.subtotal >= SHIPPING_THRESHOLD or order.subtotal == 0)
                else SHIPPING_COST
            )

            order.total_amount = (
                max(Decimal("0.00"), order.subtotal - order.coupon_discount)
                + order.shipping_fee
            )
    ...
    def calculate_item_refund(cls, item: OrderItem) -> Decimal:
        ...
        if order.coupon_discount > Decimal("0.00"):
            item_coupon_share = (
                item.line_total / total_items_subtotal
    ...
    def request_item_return(cls, user, item_id, reason, description=None):
        ...
        if order.order_status != Order.OrderStatus.DELIVERED:
            raise ValidationError(
                "Return requests can only be submitted for delivered items."
            )
    ...
    def generate_invoice_pdf(cls, user, order_id):
        ...
        try:
            ...
            clean_name = (
                order.shipping_name.encode("ascii", "ignore").decode("ascii")
                or "Customer"
            )
            clean_addr1 = (
            ...
            clean_city = (
                (order.shipping_city or "").encode("ascii", "ignore").decode("ascii")
            )
            clean_state = (
                (order.shipping_state or "").encode("ascii", "ignore").decode("ascii")
            )
            ...
            left_header = [
                ...
                Paragraph(
                    "Premium Collectibles & Toys Store",
                    ParagraphStyle(
                        "Tagline",
                        parent=brand_sub,
                        fontName="Helvetica-Bold",
                ...
                Paragraph(
                    "123 Playtime Lane, Brick City, CA 90210<br/>Support: support@toystore.com | www.toystore.com",
                    brand_sub,
                ),
            ]
            right_header = [
                ...
                Paragraph(
                    f"<b>Invoice #:</b> INV-{clean_num}<br/><b>Order #:</b> {clean_num}<br/><b>Date:</b> {order.created_at.strftime('%B %d, %Y')}",
                    invoice_meta_right,
                ),
            ]
            ...
            if order.shipping_address_line2:
                addr_lines.append(
                    order.shipping_address_line2.encode("ascii", "ignore").decode(
                        "ascii"
                    )
                )
            addr_lines.append(
                f"{clean_city}, {clean_state} - {order.shipping_postal_code}, {order.shipping_country}"
            )
            ...
            order_info_content = [
                ...
                Paragraph(
                    f"<b>Payment Status:</b> <font color='#10b981'><b>{order.get_payment_status_display()}</b></font>",
                    card_text,
                ),
                Paragraph(
                    f"<b>Order Status:</b> {order.get_order_status_display()}",
                    card_text,
                ),
            ]
            if order.coupon_code:
                c_code = (
                    order.coupon_code.upper().encode("ascii", "ignore").decode("ascii")
                )
                order_info_content.append(
                    Paragraph(
                        f"<b>Applied Coupon:</b> <font color='#10b981'><b>{c_code}</b></font>",
                        card_text,
                    )
                )
            order_info_content.append(
                Paragraph(
                    (
                        f"<b>Shipping Amount:</b> Rs. {order.shipping_fee:.2f}"
                        if order.shipping_fee != 0
                        else "<b>Shipping Type:</b> Express Delivery (FREE)"
                    ),
                    card_text,
                )
            )
            ...
            t_cards = Table(
                [[bill_to_content, order_info_content]], colWidths=[260, 260]
            )
            t_cards.setStyle(
                TableStyle(
                    [
            ...
            table_data = [
                [
                    ...
                    Paragraph(
                        "UNIT PRICE",
                        ParagraphStyle("RHead", parent=th_style, alignment=2),
                    ),
                    Paragraph(
                        "QTY", ParagraphStyle("CHead", parent=th_style, alignment=1)
                    ),
                    Paragraph(
                        "LINE TOTAL",
                        ParagraphStyle("RHead2", parent=th_style, alignment=2),
                    ),
                ]
            ]
            ...
            for idx, item in enumerate(items_list):
                p_name = (
                    item.product_name.encode("ascii", "ignore").decode("ascii")
                    or "Product"
                )
                v_name = (
                    item.variant_name.encode("ascii", "ignore").decode("ascii")
                    or "Default"
                )
                sku_str = f"SKU: {item.sku}" if item.sku else ""
                is_item_cancelled = (
                    item.status == OrderItem.ItemStatus.CANCELLED
                    or order.order_status == Order.OrderStatus.CANCELLED
                )
                ...
                prod_cell = [
                    Paragraph(
                        f"<b>{p_name}</b>"
                        + (
                            " <font color='#ef4444'><b>[CANCELLED]</b></font>"
                ...
                if is_item_cancelled and item.cancellation_reason:
                    c_reason = item.cancellation_reason.encode(
                        "ascii", "ignore"
                    ).decode("ascii")
                    prod_cell.append(
                        Paragraph(
                            f"<font color='#ef4444'>Reason: {c_reason}</font>", tb_muted
                        )
                    )

                price_str = (
                    f"<s>Rs. {item.price:.2f}</s>"
                    if is_item_cancelled
                    else f"Rs. {item.price:.2f}"
                )
                line_total_str = (
                    "<font color='#ef4444'><b>CANCELLED</b></font>"
                    if is_item_cancelled
                    else f"<b>Rs. {item.line_total:.2f}</b>"
                )
                ...
                table_data.append(
                    [
                        ...
                        Paragraph(
                            v_name
                            + (
                                " <font color='#ef4444'>(Cancelled)</font>"
                                if is_item_cancelled
                                else ""
                            ),
                            tb_muted,
                        ...
                        Paragraph(
                            price_str,
                            ParagraphStyle("RCell", parent=tb_style, alignment=2),
                        ),
                        Paragraph(
                            str(item.quantity),
                            ParagraphStyle("CCell", parent=tb_style, alignment=1),
                        ),
                        Paragraph(
                            line_total_str,
                            ParagraphStyle("RCellBold", parent=tb_style, alignment=2),
                        ),
                    ]
                )
            ...
            totals_data = [
                [
                    Paragraph("Subtotal:", total_lbl),
                    Paragraph(f"Rs. {order.subtotal:.2f}", total_val),
                ],
            ]
            ...
            if order.coupon_discount > 0 or order.coupon_code:
                c_code_str = (
                    f" ({order.coupon_code.upper().encode('ascii', 'ignore').decode('ascii')})"
                    if order.coupon_code
                    else ""
                )
                totals_data.append(
            ...
            t_wrapper = Table(
                [[Paragraph("", card_text), t_totals]], colWidths=[290, 250]
            )
            t_wrapper.setStyle(
                TableStyle(
                    [
            ...
            elements.append(
                Paragraph(
                    "<b>Thank you for shopping with Toy Store! 🎁</b>",
                    ParagraphStyle(
                        "FootTitle",
                        parent=styles["Normal"],
                        alignment=1,
            ...
            elements.append(
                Paragraph(
                    "For support or returns, visit <u>www.toystore.com</u> or contact <u>support@toystore.com</u>",
                    ParagraphStyle(
                        "FootSub",
                        parent=styles["Normal"],
                        alignment=1,
            ...
            for item in order.items.all():
                ...
                if is_item_cancelled:
                    ...
                    lines.append(
                        f"   Qty: {item.quantity}  |  Price: Rs. {item.price:.2f}  |  STATUS: CANCELLED ({c_reason})"
                    )
                else:
                    lines.append(
                        f"   Qty: {item.quantity}  |  Price: Rs. {item.price:.2f}  |  Line Total: Rs. {item.line_total:.2f}"
                    )
            ...
            if order.coupon_discount > 0 or order.coupon_code:
                c_lbl = (
                    f"Coupon Savings ({clean_str(order.coupon_code.upper())})"
                    if order.coupon_code
                    else "Coupon Savings"
                )
