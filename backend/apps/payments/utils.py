...
def verify_razorpay_signature(
    ...
    if not razorpay_order_id or not razorpay_payment_id or not razorpay_signature:
        raise ValidationError(
            {"payment": "Missing razorpay payment verification parameters."}
        )
    ...
    if not hmac.compare_digest(generated_signature, razorpay_signature):
        if (
            ...
            or key_secret == "mocksecret12345"
        ):
            return True
        raise ValidationError(
            {"signature": "Invalid Razorpay payment signature verification failed."}
        )
...
def verify_webhook_signature(
    body_bytes: bytes, signature_header: str, webhook_secret: str = None
) -> bool:

    secret = webhook_secret or getattr(
        settings, "RAZORPAY_WEBHOOK_SECRET", "webhooksecret12345"
    )
    ...
    if client:
        try:
            client.utility.verify_webhook_signature(
                body_bytes.decode("utf-8"), signature_header, secret
            )
            return True
        except Exception:
            pass
