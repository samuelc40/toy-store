...
class RazorpayWebhookAPIView(APIView):
    ...
    def post(self, request):
        ...
        payment = (
            Payment.objects.filter(gateway_order_id=rzp_order_id)
        ...
        if not payment and rzp_payment_id:
            payment = (
                Payment.objects.filter(gateway_payment_id=rzp_payment_id)
                .select_related("order")
                .first()
            )
        ...
        if event == "payment.captured":
            if payment:
                ...
                payment.gateway_payment_id = (
                    rzp_payment_id or payment.gateway_payment_id
                )
