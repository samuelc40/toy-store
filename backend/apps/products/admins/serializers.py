...
class CategorySerializer(serializers.ModelSerializer):
    ...
    def validate_name(self, value):
        ...
        if len(name) > max_len:
            raise serializers.ValidationError(
                f"Category name cannot exceed {max_len} characters."
            )
...
class ProductVariantSerializer(serializers.ModelSerializer):
    ...
    def validate_variant_name(self, value):
        ...
        if len(name) > max_len:
            raise serializers.ValidationError(
                f"Variant name cannot exceed {max_len} characters."
            )
        return name
    ...
    def validate_sku(self, value):
        ...
        if len(sku) > max_len:
            raise serializers.ValidationError(
                f"SKU cannot exceed {max_len} characters."
            )
    ...
    def validate(self, attrs):
        ...
        price = (
            attrs.get("price")
            if "price" in attrs
            else (self.instance.price if self.instance else None)
        )
        sale_price = (
            attrs.get("sale_price")
            if "sale_price" in attrs
            else (self.instance.sale_price if self.instance else None)
        )
