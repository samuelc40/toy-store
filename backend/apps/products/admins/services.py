...
class AdminCategoryService:
    ...
    @staticmethod
    def list_categories(page=1, page_size=10, search=None, sort="newest"):
        ...
        categories = AdminCategoryService.apply_sort(categories, sort)
        ...
        next_page = paginated.has_next() if paginated else False

        previous_page = paginated.has_previous() if paginated else False
    ...
    def update_category(category_uuid, name=None, description=None, image=None):
        ...
        if name:
            name = name.strip()
            clash = (
                Category.objects.filter(name__iexact=name, is_active=True)
            ...
            inactive_clash = Category.objects.filter(
                name__iexact=name, is_active=False
            ).first()
            if inactive_clash:
                import uuid

                inactive_clash.name = (
                    f"{inactive_clash.name}_deleted_{uuid.uuid4().hex[:6]}"
                )
                inactive_clash.save()
...
class ProductService:
    ...
    @staticmethod
    def list_products(search="", page=1, page_size=10):
        ...
        queryset = (
            ...
            .annotate(
                ...
                total_stock=Coalesce(
                    Sum("variants__stock_quantity", filter=Q(variants__is_active=True)),
                    Value(0),
                ),
                lowest_price=Min(
    ...
    def get_product(product_id):
        ...
        return get_object_or_404(
            Product.objects.select_related("category"), id=product_id, is_active=True
        )
...
class VariantService:
    ...
    def list_variants(product):
        return ProductVariant.objects.filter(product=product, is_active=True).order_by(
    ...
    def soft_delete(variant):
        ...
        variant.save(update_fields=["sku", "is_active", "updated_at"])
        return variant

    @staticmethod
    def toggle_block(variant):

        variant.blocked = not variant.blocked

        variant.save(update_fields=["blocked", "updated_at"])
...
class ImageService:
    ...
    @staticmethod
    @transaction.atomic
    def upload_images(variant_id, images, alt_text=""):
        ...
        for image in images:
            ...
            if image.size > ImageService.MAX_IMAGE_SIZE:
                raise ValidationError({"images": f"{image.name} exceeds 5MB."})
    ...
    def delete_image(variant_id, image_id):
        ...
        image = get_object_or_404(ProductImage, id=image_id, variant=variant)
...
class AdminInventoryService:
    ...
    def get_inventory_items(
        ...
        if category and str(category).strip() and str(category).upper() != "ALL":
            ...
            queryset = queryset.filter(
                Q(product__category__id=q_cat)
                | Q(product__category__name__iexact=q_cat)
            )
        ...
        if (
            stock_status
            and str(stock_status).strip()
            and str(stock_status).upper() != "ALL"
        ):
            ...
            if st == "OUT_OF_STOCK":
                ...
                queryset = queryset.filter(
                    stock_quantity__gt=0, stock_quantity__lte=cls.STOCK_LOW_THRESHOLD
                )
            elif st == "IN_STOCK":
        ...
        if (
            status_filter
            and str(status_filter).strip()
            and str(status_filter).upper() != "ALL"
        ):
            ...
            if sf == "ACTIVE":
                ...
                queryset = queryset.filter(
                    Q(is_active=False) | Q(product__is_active=False)
                )
            elif sf == "BLOCKED":
        ...
        try:
            ...
            paginated_page = paginator.page(
                paginator.num_pages if paginator.num_pages > 0 else 1
            )
        ...
        return {
            ...
            "total_pages": paginator.num_pages,
            "next": (
                paginated_page.next_page_number() if paginated_page.has_next() else None
            ),
            "previous": (
                paginated_page.previous_page_number()
    ...
    def get_summary_stats(cls):
        ...
        in_stock = ProductVariant.objects.filter(
            stock_quantity__gt=cls.STOCK_LOW_THRESHOLD
        ).count()
