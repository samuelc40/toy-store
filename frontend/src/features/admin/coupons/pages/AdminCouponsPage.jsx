...
export function AdminCouponsPage() {
  ...
  const { coupons, count, page, pageSize, totalPages, loading, actionLoading } =
  ...
  const handleCreateSubmit = async (formData) => {
    ...
    if (editingCoupon) {
      res = await dispatch(
        updateCouponThunk({ id: editingCoupon.id, data: formData }),
      );
      if (!res.error) {
      ...
      if (!res.error) {
        setIsCreateModalOpen(false);
        dispatch(
          getAdminCoupons({ search, sort, page: 1, page_size: pageSize }),
        );
        return null;
      }
    }
  ...
  const totalUsageSum = coupons.reduce(
    (acc, c) => acc + (c.used_count || 0),
    0,
  );
    ...
    <div className="admin-coupons-page">
      ...
      <div className="coupons-header">
        <div className="coupons-title-group">
          ...
          <p>
            Create, manage and track discount coupons for customer checkout.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="coupons-desktop-wrapper">
                    ...
                    <td>
                      ...
                      {c.maximum_discount_amount &&
                        c.discount_type === "PERCENTAGE" && (
                          ...
                          >
                            Max: Rs.{" "}
                            {Number(c.maximum_discount_amount).toFixed(2)}
                          </div>
                        )}
                    </td>
                    <td>
      ...
      <div className="coupons-mobile-list">
        {loading ? (
          <div
            style={{
              textAlign: "center",
          ...
          >
            <RefreshCw
              ...
              style={{ opacity: 0.5 }}
            />
            <p style={{ margin: "8px 0 0 0" }}>Loading coupons...</p>
          </div>
        ) : coupons.length === 0 ? (
          <div
            style={{
          ...
          coupons.map((c) => (
            <div key={c.id} className="admin-coupon-card">
              ...
              {/* Discount Box */}
              <div className="coupon-card-discount-box">
                <span
                  className="discount-val-text"
                  style={{ fontSize: "15px" }}
                >
                  {c.discount_type === "PERCENTAGE"
                    ? `${c.discount_value}% OFF`
              ...
              {/* Details Grid */}
              <div className="coupon-card-details-grid">
                <div className="coupon-card-detail-item">
                  <span className="coupon-card-detail-label">Min Order</span>
                  <span className="coupon-card-detail-val">
                    {Number(c.minimum_order_amount) > 0
                      ? `Rs. ${Number(c.minimum_order_amount).toFixed(2)}`
                      : "None"}
                  </span>
                ...
                <div className="coupon-card-detail-item">
                  <span className="coupon-card-detail-label">Expiry Date</span>
                  <span className="coupon-card-detail-val">
                    {new Date(c.end_date).toLocaleDateString()}
                  </span>
                </div>
                <div
                  className="coupon-card-detail-item"
                  style={{ gridColumn: "span 2" }}
                >
                  <span className="coupon-card-detail-label">Redemptions</span>
                  <div
                    style={{
                      display: "flex",
                      alignItems: "center",
              ...
              <div className="coupon-card-actions">
                <button
                  type="button"
                  className="btn-coupon-card-action edit"
                  onClick={() => {
                    setEditingCoupon(c);
                    setIsCreateModalOpen(true);
                  }}
                ...
                <button
                  type="button"
                  className="btn-coupon-card-action delete"
                  onClick={() => setDeletingCoupon(c)}
                >
                  <Trash2 size={14} />
