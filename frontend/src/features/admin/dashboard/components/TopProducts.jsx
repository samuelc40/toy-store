        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '13px' }}>
          <thead>
            <tr style={{ backgroundColor: 'var(--bg-secondary, rgba(0, 0, 0, 0.02))', borderBottom: '1px solid var(--border-color)' }}>
              ...
              <th style={{ padding: '12px 16px', fontWeight: 800, color: 'var(--text-secondary)' }}>Product Name</th>
          ...
          <tbody>
            {products.map((item) => (
              <tr key={item.rank} style={{ borderBottom: '1px solid var(--border-color)' }}>
                <td style={{ padding: '12px 16px', fontWeight: 800, color: item.rank <= 3 ? 'var(--accent)' : 'var(--text-secondary)' }}>
                  {item.rank}
                </td>
