...
export default function CategoryOfferFormModal({
  isOpen,
  onClose,
  onSubmit,
  initialData = null,
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (initialData) {
        setCategoryId(
          initialData.category?.id || initialData.category_id || "",
        );
  ...
  const fetchCategories = async () => {
    try {
      ...
      let response;
      try {
        response = await api.get("/admin/products/categories/", {
          params: { page_size: 100 },
        });
      } catch (err) {
        response = await api.get("/customers/categories/", {
          params: { page_size: 100 },
        });
      }
      ...
      <div className="admin-modal-container" style={{ maxWidth: "560px" }}>
        ...
        <form onSubmit={handleSubmit} className="admin-modal-body">
          ...
          <div className="admin-form-group">
            ...
            {errors.categoryId && (
              <span className="admin-form-error">{errors.categoryId}</span>
            )}
          </div>
          ...
          <div className="admin-modal-grid-2col">
            ...
            <div className="admin-form-group">
              ...
              <input
                ...
                placeholder={
                  discountType === "PERCENTAGE" ? "e.g. 20" : "e.g. 150"
                }
                value={discountValue}
              ...
              {errors.discountValue && (
                <span className="admin-form-error">{errors.discountValue}</span>
              )}
            </div>
          </div>
          ...
          <div className="admin-modal-grid-2col">
            <div className="admin-form-group">
              ...
              />
              {errors.startDate && (
                <span className="admin-form-error">{errors.startDate}</span>
              )}
            </div>
            ...
            <div className="admin-form-group">
              ...
              />
              {errors.endDate && (
                <span className="admin-form-error">{errors.endDate}</span>
              )}
            </div>
          </div>
          ...
          >
            ...
            />
            <label
              htmlFor="category-offer-active"
              style={{ cursor: "pointer", fontSize: "13.5px", fontWeight: 600 }}
            >
