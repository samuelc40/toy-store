...
import { X, Calendar, Percent, Tag, Package } from "lucide-react";
import api from "../../../../api/axios";

export default function ProductOfferFormModal({
  isOpen,
  onClose,
  onSubmit,
  initialData = null,
            ...
            >
              ...
              {products.map((p) => (
                <option key={p.id} value={p.id}>
                  {p.name} ({p.brand || "Brand"}) -{" "}
                  {p.category_name || p.category?.name || "Category"}
                </option>
              ))}
            </select>
            {errors.productId && (
              <span className="admin-form-error">{errors.productId}</span>
            )}
          </div>
