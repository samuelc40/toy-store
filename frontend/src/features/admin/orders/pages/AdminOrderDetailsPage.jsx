...
export function AdminOrderDetailsPage() {
  ...
  const handleStatusConfirm = async (newStatus) => {
    ...
    try {
      ...
      toast.success(
        `Order #${order.order_number} status updated to '${newStatus.replace("_", " ")}'!`,
      );
              ...
              {order.items?.map((item) => (
                ...
                >
                  ...
                  <div style={{ textAlign: "right" }}>
                    ...
                    >
                      ₹{parseFloat(item.price || 0).toFixed(2)} &times;{" "}
                      {item.quantity}
                    </span>
                  </div>
                </div>
          ...
          <div className="detail-card">
            ...
            >
              ...
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                ...
                <strong
                  style={{
                    color: order.shipping_fee === 0 ? "#10b981" : "inherit",
                  }}
                >
              ...
              <hr
                style={{ borderColor: "var(--border-color)", margin: "8px 0" }}
              />
              <div
                style={{
                  display: "flex",
          ...
          {order.return_requests?.length > 0 && (
            <div className="detail-card return-notice-card">
              ...
              {order.return_requests.map((rr) => (
                <div key={rr.id}>
                  <span>
                    Reason: <strong>{rr.reason}</strong>
                  </span>
                  {rr.description && (
                    <p style={{ margin: "2px 0" }}>{rr.description}</p>
                  )}
                  <span style={{ fontSize: "11.5px" }}>
                    Status: <strong>{rr.status}</strong>
                  </span>
                </div>
              ))}
            </div>
