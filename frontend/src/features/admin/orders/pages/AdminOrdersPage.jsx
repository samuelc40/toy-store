...
export function AdminOrdersPage() {
  ...
  const handleStatusConfirm = async (newStatus) => {
    ...
    try {
      ...
      toast.success(
        `Order #${selectedOrderForStatus.order_number} status updated to '${newStatus.replace("_", " ")}'!`,
      );
      setSelectedOrderForStatus(null);
      loadOrders();
    } catch (err) {
      toast.error(
        typeof err === "string" ? err : "Failed to update order status.",
      );
    }
  };
    ...
    <div className="admin-orders-page-container">
      ...
      <div className="admin-orders-header">
        <div>
          ...
          <p>
            Track, filter, and process customer orders across all fulfillment
            stages.
          </p>
        </div>
            ...
            <tbody>
              {orders.map((ord) => (
                <tr key={ord.id}>
                  <td
                    data-label="ORDER #"
                    style={{ fontWeight: 800, color: "var(--accent-color)" }}
                  >
                    #{ord.order_number}
                  </td>
                  <td
