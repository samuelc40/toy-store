  ...
  return (
    <div className="product-modal-backdrop-overlay">
      <div className="product-confirm-dialog-card animate-scale-in">
        ...
        <div
          className={`confirm-dialog-icon-container ${isBlocking ? "bg-warning-light text-warning" : "bg-success-light text-success"}`}
        >
          <ShieldAlert size={32} />
        </div>
        ...
        <p className="confirm-dialog-message">
          {isBlocking ? (
            <>
              ...
              Blocking this product will hide all its listings and variants from
              customer views.
            </>
          ) : (
            <>
              Are you sure you want to unblock <strong>{product.name}</strong>?
              This will restore the product and its active variants back to
              customer pages.
            </>
          )}
        </p>
