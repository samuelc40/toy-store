  ...
  return (
    <div className="variant-modal-backdrop-overlay">
      <div className="variant-confirm-dialog-card animate-scale-in">
        ...
        <div
          className={`v-confirm-dialog-icon-container ${isBlocking ? "bg-warning-light text-warning" : "bg-success-light text-success"}`}
        >
          <ShieldAlert size={28} />
        </div>
        ...
        <p className="v-confirm-dialog-message">
          {isBlocking ? (
            <>
              Are you sure you want to block variant{" "}
              <strong>{variant.variant_name}</strong> (SKU:{" "}
              <em>{variant.sku}</em>)? Customers will not be able to purchase or
              view this specific variant selection.
            </>
          ) : (
            <>
              Are you sure you want to unblock variant{" "}
              <strong>{variant.variant_name}</strong> (SKU:{" "}
              <em>{variant.sku}</em>)? This restores item visibility and
              purchasing in the shop interface.
            </>
          )}
        </p>
