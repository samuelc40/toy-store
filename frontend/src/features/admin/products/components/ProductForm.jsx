...
export function ProductForm({
  ...
  const toggleCardExpansion = (vIndex) => {
    setLocalVariants((prev) =>
      prev.map((v, idx) =>
        idx === vIndex ? { ...v, expanded: !v.expanded } : v,
      ),
    );
  };
    ...
    setLocalVariants((prev) => {
      ...
      if (field === "sku") {
        ...
        return updatedList.map((v) => {
          const s = (v.sku || "").trim().toLowerCase();
          if (
            v.errors?.sku === "Duplicate SKU in variant list." &&
            (!s || skuCounts[s] <= 1)
          ) {
  ...
  const handleFilesAddedForVariant = (vIndex, files) => {
    ...
    const validItems = fileList
      .filter((file) => {
        ...
        if (!isValid) {
          toast.warning(
            `File "${file.name}" is rejected (max 5MB, PNG/JPG/WebP only).`,
          );
        }
        return isValid;
      })
      ...
      prev.map((v, idx) => {
        if (idx === vIndex) {
          const totalImagesCount =
            v.images.length + v.newImagesQueue.length + validItems.length;
        ...
        if (idx === vIndex) {
          const updatedQueue = v.newImagesQueue.map((item, fIdx) => {
            if (fIdx === fileIndex) {
              ...
              const nameWithoutExt = item.file.name.substring(
                0,
              ...
              const croppedFile = new File(
                [actualBlob],
                `${nameWithoutExt}-cropped${extension}`,
                {
                  type: item.file.type,
      ...
      prev.map((v, idx) => {
        if (idx === vIndex) {
          ...
          const updatedDeletedIds = Array.from(
            new Set([...(v.deletedImageIds || []), imageId]),
          );
  ...
  const handleSetPrimaryImage = async (vIndex, imageId, variantId) => {
    ...
    const result = await dispatch(
      setVariantPrimaryImageAsync({ variantId, imageId }),
    );
  ...
  const validateVariantsData = () => {
    ...
    const updated = localVariants.map((v) => {
      ...
      if (
        v.display_order === "" ||
        isNaN(Number(v.display_order)) ||
        Number(v.display_order) <= 0
      ) {
    ...
    if (hasErrors) {
      ...
      const errIndex = updated.findIndex((v) =>
        Object.values(v.errors || {}).some(Boolean),
      );
      ...
      toast.error(
        "Please correct the validation errors in the product variant cards.",
      );
      return false;
    }
  ...
  return (
    <>
      <form
        ...
        className="product-form-el unified-product-workflow-form"
      >
        <div className="product-info-fields-section">
          ...
          <div className="form-fields-grid-layout">
            ...
            <div className="form-group-field">
              ...
              {errors.category && (
                <p className="form-field-error">{errors.category.message}</p>
              )}
            </div>
            ...
            <div className="form-group-field">
              ...
              />
              {errors.brand && (
                <p className="form-field-error">{errors.brand.message}</p>
              )}
            </div>
          </div>
        ...
        <div className="product-form-variants-section-block">
          <div className="variants-section-header-el">
            <div className="variants-section-title-wrapper">
              ...
              <span
                className="info-helper-tooltip"
                title="Every variant requires 3-10 product images. Price, Sale Price, and Stock will be validated before saving."
              >
                <HelpCircle size={14} />
              </span>
            </div>
          ...
          {loadingVariants && localVariants.length === 0 ? (
            ...
            <div className="variants-empty-state-card">
              <p className="empty-text">
                No variants added to this product form yet.
              </p>
              <button
                type="button"
                onClick={handleAddVariantDraft}
            ...
            <div className="expandable-variants-cards-stack">
              {localVariants.map((v, vIndex) => {
                const hasCardErrors = Boolean(
                  v.errors && Object.values(v.errors).some(Boolean),
                );
                return (
                  <div
                    key={v.id}
                    ...
                    >
                      <div className="variant-header-info-box">
                        ...
                        <span className="v-card-name-title">
                          {v.variant_name || (
                            <em className="untitled-text-dim">
                              Untitled Variant Card
                            </em>
                          )}
                        </span>
                        {v.sku && (
                          <span className="v-card-sku-badge">{v.sku}</span>
                        )}
                      </div>
                      <div
                        className="variant-header-actions-box"
                        onClick={(e) => e.stopPropagation()}
                      >
                        {v.blocked && (
                          <span className="badge-status-v blocked">
                            Blocked
                          </span>
                        )}
                        {!v.is_active && (
                          <span className="badge-status-v inactive">
                            Inactive
                          </span>
                        )}
                        {!isReadOnly && (
                      ...
                      <div className="variant-card-expanded-content">
                        <div className="variant-inputs-grid-layout">
                          ...
                          <div className="form-group-field">
                            ...
                            />
                            {v.errors?.sku && (
                              <p className="form-field-error">{v.errors.sku}</p>
                            )}
                          </div>
                        ...
                        <div className="variant-inline-images-workspace">
                          ...
                          {v.images && v.images.length > 0 && (
                            <div className="v-inline-images-gallery">
                              <ImageGallery
                                images={v.images}
                                onDelete={(imgId) =>
                                  handleDeleteImage(vIndex, imgId, v.id)
                                }
                                onSetPrimary={(imgId) =>
                                  handleSetPrimaryImage(vIndex, imgId, v.id)
                                }
                                disabled={isReadOnly}
                              />
                            </div>
                          ...
                          {!isReadOnly && (
                            <div className="v-inline-images-uploader">
                              <ImageUploader
                                variantId={
                                  v.id && !String(v.id).startsWith("draft-")
                                    ? v.id
                                    : null
                                }
                                ...
                                queueController={{
                                  ...
                                  activeCropIndex:
                                    activeCropInfo &&
                                    activeCropInfo.vIndex === vIndex
                                      ? activeCropInfo.fileIndex
                                      : null,
                                  handleFilesAdded: (files) =>
                                    handleFilesAddedForVariant(vIndex, files),
                                  removeFile: (fileId) =>
                                    handleRemoveQueuedFile(vIndex, fileId),
                                  openCropper: (fileIndex) =>
                                    setActiveCropInfo({ vIndex, fileIndex }),
