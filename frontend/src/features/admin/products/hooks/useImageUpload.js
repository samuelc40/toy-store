...
export const useImageUpload = (
      ...
      if (newQueueItems.length > 0) {
        ...
        for (const item of newQueueItems) {
          try {
            setQueue((prev) =>
              prev.map((q) =>
                q.id === item.id ? { ...q, status: "compressing" } : q,
              ),
            );
  ...
  const saveCroppedImage = useCallback((index, croppedBlob) => {
    setQueue((prev) => {
      ...
      const nameWithoutExt = target.file.name.substring(
        0,
        target.file.name.lastIndexOf("."),
      );
  ...
  const uploadImages = useCallback(async () => {
    ...
    if (totalImagesCount < 3) {
      toast.warning(
        `A variant must have at least 3 images. You need to upload ${3 - existingImagesCount} more.`,
      );
      return;
    }
