...
export const isItemBlocked = (item) => {
  ...
  if (
    v.product &&
    (v.product.blocked === true || v.product.is_active === false)
  )
    return true;
  return false;
};
...
export function CartPage() {
  ...
  const shippingCost =
    summary?.shipping_fee !== undefined
  ...
  const hasBlockedItems =
    summary?.has_blocked_items || items.some(isItemBlocked);
  ...
  const isCheckoutDisabled =
    items.length === 0 || hasOutofStockItems || hasBlockedItems;

  const handleCheckout = async () => {
    if (isCheckoutDisabled) {
      if (hasBlockedItems) {
        toast.error(
          "Your cart contains unavailable or blocked items. Please remove them to proceed.",
        );
      } else {
        toast.error(
          "Cannot proceed to checkout. Please resolve out-of-stock items.",
        );
      }
      return;
    }
  ...
  const getProductUrl = (item) => {
    const productId =
      item?.variant?.product_id || item?.variant?.product?.id || "";
    const variantId = item?.variant?.id || "";
    if (!productId) return "#";
    return `/products/${productId}${variantId ? `?variant=${variantId}` : ""}`;
  };
    ...
    return (
      <div className="cart-page-outer-container">
        <div className="cart-empty-state-outer-wrapper">
          <span
            className="cart-empty-visual-illustration"
            role="img"
          ...
          <p className="cart-empty-subdescription">
            ...
            top categories or new arrivals to find the perfect toy!
          </p>
        ...
        <div className="cart-items-list-workspace">
          ...
          {items.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className={`cart-item-card-wrapper ${isBlocked ? "is-item-blocked" : ""}`}
                onClick={(e) => handleCardClick(e, item)}
                style={{ cursor: "pointer" }}
              >
                <div className="cart-item-content-body">
                  {/* Thumbnail */}
                  <Link to={productUrl} className="cart-item-thumbnail-box">
                    <img
                  ...
                  <div className="cart-item-details-stack">
                    <Link to={productUrl} className="cart-item-title-name">
                      {item.variant?.product_name || "Product"}
                    </Link>
                    <Link
                      to={productUrl}
                      className="cart-item-variant-label"
                    ...
                    {isBlocked ? (
                      <span
                        style={{ color: "red" }}
                        className="cart-item-stock-warning-label blocked-item"
                      >
                    ...
                    {(() => {
                      ...
                      const hasDiscount =
                        discountPct > 0 ||
                        Number(originalUnitPrice) > Number(currentUnitPrice);
                        ...
                        <div className="cart-item-pricing-column">
                          ...
                          {hasDiscount && (
                            <>
                              ...
                              <span
                                className="discount-badge-green"
                ...
                <div className="cart-item-action-footer">
                  <div className="cart-item-quantity-controls-block">
                    ...
                    <span className="cart-item-qty-value-display">
                      {item.quantity}
                    </span>
                    <button
                      type="button"
                  ...
                  <div className="cart-item-subtotal-label">
                    <span className="cart-item-subtotal-lbl-prefix">
                      Total:{" "}
                    </span>
        ...
        <div className="cart-order-summary-card">
          ...
          <div className="summary-details-rows-stack">
            ...
            {shippingCost > 0 && (
              ...
              >
                Add {formatPrice(shippingThreshold - subtotal)} more for FREE
                shipping!
              </span>
            )}
          </div>
          ...
          {hasBlockedItems && (
            <div className="cart-checkout-warning-box">
              ...
              <span>
                {" "}
                Unavailable items in cart. Please remove them before checkout.
              </span>
            </div>
          )}
  ...
  return (
    <div className="cart-page-outer-container details-page-skeleton-loader">
      <div
        ...
        style={{ width: "250px", height: "36px", marginBottom: "32px" }}
      />
      <div className="cart-page-split-layout">
        <div className="cart-items-list-workspace">
          {[...Array(3)].map((_, i) => (
