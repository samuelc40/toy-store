  ...
  const baseSlides =
    backendProducts.length > 0
      ? backendProducts.map((p, idx) => ({
          ...
          name: p.name,
          tagline:
            p.description || p.category || "Exclusive Toy Store Selection",
          price: p.lowest_price !== undefined ? p.lowest_price : p.price || 999,
          original_price: p.original_price || null,
          badge: heroData?.discount
            ? `🔥 ${heroData.discount}`
            : idx === 0
              ? "🔥 SPECIAL OFFER"
              : "⭐ FEATURED",
          image:
            p.primary_image ||
            [cyberToyCar, catMonsterTruck, prodMech, prodDrone][idx % 4],
    ...
    >
      ...
      <div className="hero-slider-header">
        <div className="hero-slider-header-left">
          <span className="hero-slider-badge-pill">{currentSlide.badge}</span>
          <button
            type="button"
          ...
          {typeof currentSlide.price === "number" ? (
            <>
              ...
              {currentSlide.original_price && (
                <span className="slider-orig-price">
                  ₹{currentSlide.original_price}
                </span>
              )}
            </>
          ) : (
            <span className="slider-current-price offer-highlight-text">
              {currentSlide.price}
            </span>
          )}
        </div>
