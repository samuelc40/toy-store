    ...
    <div className="customer-order-card">
      ...
      <div className="order-card-header">
        <div className="order-card-header-top">
          ...
          <span
            className={`order-status-pill ${getStatusPillClass(order?.order_status)}`}
          >
        ...
        <div className="order-card-items-list-container">
          {items.map((item, idx) => {
            ...
            return (
              <div key={item.id || idx} className="order-card-item-row">
                <Link to={productUrl} className="order-card-thumbnail-box">
                  {item.image ? (
                    <img
                      src={item.image}
                ...
                <div className="order-card-info-group">
                  ...
                  <p className="order-items-count-text">
                    {item.variant_name && (
                      <span>Edition: {item.variant_name} • </span>
                    )}
                    <span className="order-item-qty-tag">
                      Qty: {item.quantity || 1}
                    </span>
                  </p>
                </div>
          ...
          <div className="order-payment-meta-row">
            <span className="payment-method-text">
              ...
              {order.payment_status || "Pending"}) • {totalItems}{" "}
