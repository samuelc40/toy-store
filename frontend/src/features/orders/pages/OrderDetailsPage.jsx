...
export function OrderDetailsPage() {
  ...
  const handleConfirmCancelItem = async (reason) => {
    ...
    try {
      await dispatch(
        cancelOrderItemAsync({ itemId: cancellingItem.id, reason }),
      ).unwrap();
      toast.success(
        `Item "${cancellingItem.product_name}" cancelled successfully.`,
      );
  ...
  const handleConfirmReturn = async ({ reason, description }) => {
    try {
      ...
      if (returningItem) {
        ...
        toast.success(
          res.message ||
            `Return request for "${returningItem.product_name}" submitted successfully.`,
        );
      } else {
        res = await dispatch(
      ...
      toast.error(
        typeof err === "string" ? err : "Failed to submit return request.",
      );
    }
  };
      ...
      <div className="orders-page-outer-container">
        <div className="orders-empty-state-card">
          ...
          <button
            ...
            className="btn-explore-toys-orders"
          >
            <ArrowLeft size={16} />
    ...
    <div className="orders-page-outer-container">
      ...
      <div className="order-details-top-bar">
        <button
          ...
          aria-label="Navigate back to my orders list"
        >
          <ArrowLeft size={16} />
      ...
      {returnRequests.length > 0 && (
        <div className="return-request-alert-card">
          ...
          <p>
            <strong>Reason:</strong> {returnRequests[0].reason}
          </p>
          {returnRequests[0].description && (
            <p>
              <strong>Details:</strong> {returnRequests[0].description}
            </p>
          )}
          <span className="return-date-tag">
            Requested on{" "}
            {new Date(returnRequests[0].requested_at).toLocaleDateString()}
          </span>
        </div>
      )}
          ...
          <div className="order-details-section-card">
            ...
            <div className="details-items-table-wrapper">
              {items.map((item) => {
                ...
                const isItemReturnRequested =
                  item.status === "RETURN_REQUESTED" ||
                  item.return_request?.status === "PENDING";
                const isItemReturnRejected =
                  item.return_request?.status === "REJECTED";
                const isItemCancelRequested =
                  item.cancellation_request?.status === "PENDING";
                const productUrl = getProductUrl(item);

                return (
                  <div
                    key={item.id}
                    className={`details-item-row ${isItemCancelled ? "item-cancelled" : ""}`}
                    onClick={(e) => handleItemClick(e, item)}
                    style={{ cursor: "pointer" }}
                  >
                    <Link to={productUrl} className="item-img-box">
                      {item.image ? (
                        <img
                          src={item.image}
                    ...
                    <div className="item-details-box">
                      ...
                      {isItemReturned && (
                        <div
                          className="item-return-badge returned"
                          style={{
                            display: "inline-flex",
                            alignItems: "center",
                            gap: "4px",
                        ...
                        >
                          <RotateCcw size={12} />
                          <span>
                            Returned (Refunded: Rs.{" "}
                            {Number(
                              item.return_request?.refund_amount ||
                                item.line_total,
                            ).toFixed(2)}
                            )
                          </span>
                      ...
                      {isItemReturnRejected && (
                        <div
                          className="item-return-badge rejected"
                          style={{
                            display: "inline-flex",
                            alignItems: "center",
                            gap: "4px",
                    ...
                    <div className="item-price-qty-box">
                      <span className="item-unit-price">
                        Rs. {Number(item.price || 0).toFixed(2)} x{" "}
                        {item.quantity}
                      </span>
            ...
            <div className="details-totals-block">
              ...
              {Number(order.discount_amount || 0) > 0 && (
                <div className="total-row discount-row">
                  ...
                  <span>
                    -Rs. {Number(order.discount_amount || 0).toFixed(2)}
                  </span>
                </div>
              )}
              ...
              {order.coupon_code && (
                ...
                >
                  ...
                  <span style={{ fontWeight: 800 }}>
                    -Rs.{" "}
                    {Number(
                      order.coupon_discount !== undefined &&
...
function OrderDetailsSkeleton() {
  return (
    <div className="orders-page-outer-container">
      ...
      <div
        className="skeleton-shimmer"
        style={{ width: "100%", height: "300px", borderRadius: "18px" }}
      />
    </div>
  );
}
