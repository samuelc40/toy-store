...
const extractErrorMessage = (error, defaultMsg) => {
  ...
  if (data.reason)
    return Array.isArray(data.reason) ? data.reason.join(" ") : data.reason;
...
export const cancelOrderItemAsync = createAsyncThunk(
  ...
  async ({ itemId, reason }, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        extractErrorMessage(error, "Failed to cancel order item."),
      );
    }
  },
);
...
export const requestReturnAsync = createAsyncThunk(
  ...
  async ({ orderId, reason, description }, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        extractErrorMessage(error, "Failed to submit return request."),
      );
    }
  },
);
...
export const requestItemReturnAsync = createAsyncThunk(
  ...
  async ({ itemId, reason, description }, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        extractErrorMessage(error, "Failed to submit item return request."),
      );
    }
  },
);
