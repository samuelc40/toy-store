...
export function ProductCard({ product }) {
  ...
  return (
    ...
    >
      <div
        className={`cust-product-card ${isOutOfStock ? "is-out-of-stock" : ""}`}
      >
        {/* Media Container */}
        ...
        <div className="card-details-box">
          ...
          >
            <div style={{ display: "flex", gap: "6px", alignItems: "center" }}>
              <span className="card-category-lbl">
                {product.category || "Toy"}
              </span>
              {product.brand && (
                <span className="card-brand-lbl">{product.brand}</span>
              )}
            </div>
            {avgRating > 0 ? (
              <div
          ...
          <div className="card-price-stock-row">
            ...
            <span
              className={`card-stock-status-lbl ${isOutOfStock ? "is-out" : totalStock <= 5 ? "is-low" : "is-in"}`}
            >
              {isOutOfStock
                ? "Sold Out"
                : totalStock <= 5
