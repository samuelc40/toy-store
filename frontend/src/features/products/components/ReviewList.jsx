...
export function ReviewList({
  ...
  const loadVariantReviews = useCallback(
    async (targetVariantId, targetPage, isCancelled) => {
      if (
        !targetVariantId ||
  ...
  const loadEligibility = useCallback(
    async (targetVariantId, isCancelled) => {
      if (
        !targetVariantId ||
        targetVariantId === "undefined" ||
        targetVariantId === "null" ||
  ...
  const breakdown =
    statistics.rating_breakdown || statistics.rating_distribution || {};
  const hasReviewed =
    eligibility && (eligibility.has_reviewed || eligibility.already_reviewed);
  const existingReview =
    eligibility &&
    (eligibility.existing_review ||
      (eligibility.existing_review_id
        ? { id: eligibility.existing_review_id }
        : null));

  return (
    <section
      className="reviews-breakdown-section-wrapper"
    ...
    >
      ...
      {!isAuthenticated ? (
        ...
        >
          ...
          <span>
            {eligibility.reason ||
              "You can review this variant after purchasing and receiving your order."}
          </span>
        </div>
      ) : null}
