...
export const getProductPrimaryImage = (product) => {
  ...
  const primaryVar =
    variantsToUse.find((v) => v.is_primary) || variantsToUse[0];
  if (primaryVar && primaryVar.images && primaryVar.images.length > 0) {
    const primaryImg =
      primaryVar.images.find((img) => img.is_primary) || primaryVar.images[0];
...
export const filterAndSortProducts = (
  ...
  result = result.filter(
    (p) => p.variants && p.variants.some((v) => v.is_active && !v.blocked),
  );
